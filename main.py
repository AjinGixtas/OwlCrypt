...
general_group.add_argument('-s', '--script', type=str, default='lsb', help='Script can be used to define custom behavior for hiding data. In all honesty, if you can use this stupid API, love yourself and build your own steganography app :/')
general_group.add_argument('-a', '--args', nargs='*', type=str, help='Pass in custom args to your --script, all data is interperted as string and seperate by white-space unless covered by quote')
...
if image.size[0] * image.size[1] * (1 if image.mode == '1' or image.mode == 'L' else 3 if image.mode == 'RGB' or image.mode == 'YCbCr' else 4 if image.mode == 'RGBA' or image.mode == 'CMYK' else 0) < len(args.data) and not args.warning: 
    print('Data is too big to fit in the image. Make the image bigger or reduce the data size. Overflowed data will be cropped')
...
if args.output_path == None:
    ...
    args.output_path = join(dirname(args.input_path), filename + '-modified' + file_extension)
...
image = stego_tools.execute(image, args.data, open(f'scripts/{args.script}.py', 'r').read(), args.args)
